//
//  UIColorExtension.m
//  BattleApp
//
//  Created by mocca on 2015. 11. 2..
//  Copyright (c) 2015년 mocca. All rights reserved.
//

#import "UIColorExtension.h"

@implementation UIColor (UIColorExtension)

+ (UIColor *) baWhiteColor {
    return [UIColor colorWithWhite:220/255.0f alpha:1];
}

+ (UIColor *) cloudColor {
    return [UIColor colorWithRed:0.910f green:0.925f blue:0.933f alpha:1.00f];
}

+ (UIColor *) silverColor {
    return [UIColor colorWithRed:0.690f green:0.714f blue:0.733f alpha:1.00f];
}

+ (UIColor *) baDarkGrayColor {
    return [UIColor colorWithWhite:108/225.0f alpha:1];
}

+ (UIColor *) concreteColor {
    return [UIColor colorWithRed:0.486f green:0.549f blue:0.553f alpha:1.00f];
}

+ (UIColor *) germanGreyColor {
    //return [UIColor colorWithRed:0.227f green:0.263f blue:0.349f alpha:1.00f];
    return [UIColor colorWithRed:0.167f green:0.194f blue:0.257f alpha:1.00f];
}

+ (UIColor *) asbestosColor {
    return [UIColor colorWithRed:0.424f green:0.475f blue:0.478f alpha:1.00f];
}

+ (UIColor *) alizalinColor {
    return [UIColor colorWithRed:0.910f green:0.302f blue:0.192f alpha:1.00f];
}

+ (UIColor *) lightCoralColor {
    return [UIColor colorWithRed:0.961f green:0.298f blue:0.333f alpha:1.00f];
}

+ (UIColor *) pomegranateColor {
    return [UIColor colorWithRed:0.678f green:0.157f blue:0.110f alpha:1.00f];
}

+ (UIColor *) pumpkinColor {
    return [UIColor colorWithRed:0.769f green:0.255f blue:0.000f alpha:1.00f];
}

+ (UIColor *) carrotColor {
    return [UIColor colorWithRed:0.769f green:0.255f blue:0.000f alpha:1.00f];
}

+ (UIColor *) baYellowColor {
    return [UIColor colorWithRed:0.988f green:0.827f blue:0.000f alpha:1.00f];
}

+ (UIColor *) sunflowerColor {
    return [UIColor colorWithRed:0.929f green:0.725f blue:0.059f alpha:1.00f];
}

+ (UIColor *) baOrangeColor {
    return [UIColor colorWithRed:0.933f green:0.541f blue:0.067f alpha:1.00f];
}

+ (UIColor *) dodgerBlueColor {
    return [UIColor colorWithRed:44/255.0f green:192/255.0f blue:254/255.0f alpha:1.00f];
}

+ (UIColor *) peterBlueColor {
    return [UIColor colorWithRed:0.169f green:0.518f blue:0.827f alpha:1.00f];
}

+ (UIColor *) belizeHoleColor {
    return [UIColor colorWithRed:0.133f green:0.420f blue:0.667f alpha:1.00f];
}

+ (UIColor *) wetAsphaltColor {
    return [UIColor colorWithRed:0.114f green:0.129f blue:0.145f alpha:1.00f];
}

+ (UIColor *) greenSeaColor {
    return [UIColor colorWithRed:0.129f green:0.184f blue:0.243f alpha:1.00f];
}

+ (UIColor *) facebookBlueColor {
    return [UIColor colorWithRed:0.098f green:0.376f blue:0.675f alpha:1.00f];
}

+ (UIColor *) emeraldColor {
    return [UIColor colorWithRed:0.267f green:0.702f blue:0.765f alpha:1.00f];
}

+ (UIColor *) baPurpleColor {
    return [UIColor colorWithRed:0.427f green:0.451f blue:0.969f alpha:1.00f];
}

+ (UIColor *) oddCellColor {
    return [UIColor colorWithWhite:14/255.0f alpha:.6f];
}

+ (UIColor *) evenCellColor {
    return [UIColor colorWithWhite:50/255.0f alpha:.6f];
}

@end
